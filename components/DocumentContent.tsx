      ...
      try {
        switch (activeSection) {
          case SectionType.ABOUT:
            if (!aboutHtml) { 
                const text = await notionService.getAboutMe();
                ...
                {crafts.map((craft) => (
                    <li key={craft.id} className="flex items-baseline gap-2 p-2 hover:bg-blue-50 rounded transition-colors group overflow-hidden">
                        ...
                        <div className="flex flex-col md:flex-row md:items-baseline gap-0.5 md:gap-2 overflow-hidden">
                            <a href={craft.url} target="_blank" rel="noopener noreferrer" className="text-docs-blue underline decoration-transparent group-hover:decoration-docs-blue transition-all font-medium text-base md:text-lg truncate">{craft.title}</a>
                ...
                <div key={section.id} className="mb-8">
                    ...
                    <ul className="editor-content list-disc pl-6 space-y-2">
                        ...
                        {section.items.map((item, idx) => (
                            <li key={idx} className="text-lg text-gray-800 leading-relaxed pl-1">{item}</li>
                        ))}
